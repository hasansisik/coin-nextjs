  ...
  return (
    <header className="relative flex items-center justify-between px-4 md:px-20 py-3 border-b dark:border-gray-800">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <div className={`w-5 h-5 flex flex-col justify-center items-center relative ${isMenuOpen ? 'transform' : ''}`}>
            <span className={`block w-5 h-0.5 bg-current absolute transition-transform duration-300 ${isMenuOpen ? 'rotate-45' : '-translate-y-1.5'}`}></span>
            <span className={`block w-5 h-0.5 bg-current absolute transition-opacity duration-300 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}></span>
            <span className={`block w-5 h-0.5 bg-current absolute transition-transform duration-300 ${isMenuOpen ? '-rotate-45' : 'translate-y-1.5'}`}></span>
          </div>
        </Button>
      </div>
        ...
        <div className="fixed inset-0 bg-white dark:bg-gray-900 z-50 overflow-y-auto">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b dark:border-gray-800">
            ...
            <div className="flex items-center gap-2">
              <LightDarkToggle className="h-10 w-10 flex items-center justify-center rounded-full bg-neutral-100 dark:bg-gray-800" />
          ...
          <div className="px-5 py-2 divide-y divide-gray-100 dark:divide-gray-800">
            {user?.name && (
              <div className="py-4 flex items-center">
                ...
                <span className="text-md font-medium dark:text-white">{user.name}</span>
              </div>
            )}
            ...
            <div className="py-4">
              <div className="space-y-3 mt-3">
                <div className="flex items-center space-x-3 rounded-lg bg-neutral-100 dark:bg-gray-800 p-3">
                  {cryptoData.btc.image ? (
                    <img src={cryptoData.btc.image} alt="BTC" className="h-6 w-6" />
                  ) : (
                    <Bitcoin className="h-6 w-6 text-orange-500" />
                  )}
                  <div className="flex-1">
                    <div className="text-sm font-medium dark:text-white">Bitcoin</div>
                ...
                <div className="flex items-center space-x-3 rounded-lg bg-neutral-100 dark:bg-gray-800 p-3">
                  {cryptoData.eth.image ? (
                    <img src={cryptoData.eth.image} alt="ETH" className="h-6 w-6" />
                  ) : (
                    <svg
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-medium dark:text-white">Ethereum</div>
            ...
            {footer?.socialMenu?.length > 0 && (
              <div className="py-4">
                <div className="mb-2">
                  <span className="font-medium dark:text-white">Bizi Takip Edin</span>
                </div>
                <div className="flex flex-wrap gap-3 justify-between mt-3">
                  {footer.socialMenu.map((social: any) => (
                    ...
                    >
                      ...
                      <span className="text-sm dark:text-white">{social.title}</span>
                    </button>
                  ))}
                </div>
              </div>
